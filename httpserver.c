...
void handle_files_request(int fd) {
  ...
  if (stat(path, &file_stats) == -1) {
    // This block runs if the file or directory does not exist.
    // We need to send a 404 Not Found response here.
    http_start_response(fd, 404);
    http_send_header(fd, "Content-Length", "0");
    http_end_headers(fd);
  } else {
    if (S_ISDIR(file_stats.st_mode)) {
      // If the path is a directory, we need to serve it differently.
      // Check if index.html exists in the directory.
      char index_path[1024];
      snprintf(index_path, sizeof(index_path), "%s/index.html", path);
      if (stat(index_path, &file_stats) == 0) {
        // index.html exists, serve it.
        serve_file(fd, index_path);
      } else {
        // index.html does not exist, serve the directory listing.
...
char *USAGE = "Usage: ./httpserver --files some_directory/ [--port 8000 "
              "--num-threads 5]\n"
              "       ./httpserver --proxy inst.eecs.berkeley.edu:80 [--port "
              "8000 --num-threads 5]\n";
